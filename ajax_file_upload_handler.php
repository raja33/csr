...
//echo "<br/>this". __FUNCTION__ ."<br/>:".json_encode($uploadedID);
						$tp->assign(array(
							'id' 	=> $uploadedFieldId,
							'value'	=> array(
								'file_url'        => $this->propertyValue['file_url'],
			...
			case 'upload_profile_picture':
				...
				if (!$validation) {
					...
					if (isset($this->propertyValue['file_id'])) {
						...
						// SET VALUE TO TEMPORARY SESSION STORAGE
						$tmpUploadsStorage = SJB_Session::getValue('tmp_uploads_storage_profile');
						$fileValue = array(
							'file_id'    => $this->fileUniqueId,
							'saved_name' => $this->propertyValue['saved_file_name'],
						);
						$tmpUploadsStorage = SJB_Array::setPathValue($tmpUploadsStorage, "{$formToken}/{$uploadedFieldId}", $fileValue);
						SJB_Session::setValue('tmp_uploads_storage_profile', $tmpUploadsStorage);
			...
			case 'get_file_field_data':
				...
				if($fieldId == 'picture')
				{
					$filesFromTmpStorage = SJB_Session::getValue('tmp_uploads_storage_profile');
				}
				else
				{
					$filesFromTmpStorage = SJB_Session::getValue('tmp_uploads_storage');
				}
				//print_r($filesFromTmpStorage);
				
				// edited by Arun Srivastava
				if($fieldId == 'picture' && !empty($filesFromTmpStorage))
				{				
					$fileUniqueId = SJB_Array::getPath($filesFromTmpStorage, "{$formToken}/{$fieldId}/file_id");
					$userInfo     = SJB_UserManager::getCurrentUserInfo();
					$usernewid    = 'Picture_'.$userInfo['sid'];					
					
					SJB_DB::query("UPDATE `uploaded_files` SET `id` = ?s WHERE `id` = ?s", $usernewid, $fileUniqueId);
					
					SJB_Session::unsetValue('tmp_uploads_storage_profile');
					
					$fileUniqueId = $usernewid;
				}
				elseif($fieldId == 'picture' && empty($filesFromTmpStorage))
				{
					$userInfo     = SJB_UserManager::getCurrentUserInfo();
					$fileUniqueId = 'Picture_'.$userInfo['sid'];
				}
				elseif($fieldId == 'Resume' && !empty($filesFromTmpStorage))
				{				
					foreach($filesFromTmpStorage as $key => $daata)
					{
						if(isset($daata['Resume']['file_id']))
						{
							$fileUniqueId = $daata['Resume']['file_id'];
						}
					}
					
					//$fileUniqueId = SJB_Array::getPath($filesFromTmpStorage, "{$formToken}/{$fieldId}/file_id");
					$userInfo     = SJB_UserManager::getCurrentUserInfo();
					$usernewid    = 'Resume_'.$userInfo['sid'];					
					
					SJB_DB::query("UPDATE `uploaded_files` SET `id` = ?s WHERE `id` = ?s", $usernewid, $fileUniqueId);
					
					SJB_DB::query("UPDATE `listings` SET `Resume` = ?s WHERE `Resume` = ?s", $usernewid, $fileUniqueId);
					
					SJB_Session::unsetValue('tmp_uploads_storage');
				...
				{
					$userInfo     = SJB_UserManager::getCurrentUserInfo();
					$fileUniqueId = 'Resume_'.$userInfo['sid'];
				}
				...
				switch ($fieldType) {
					...
					case 'logo':
						...
						break;
					case 'picture':
						$template = '../../ElegantView/users/upload_image.tpl';
						break;
					default:
						break;
				}
